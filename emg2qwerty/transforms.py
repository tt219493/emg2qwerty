...
class LowPassFilter:
    ...
    def __post_init__(self) -> None:
        ...
        nyquist = 0.5 * 2000  # Nyquist frequency (half of the sampling rate)
        normal_cutoff = self.cutoff / nyquist  # Normalize the cutoff frequency

        # Create a low-pass Butterworth filter (4th order)
        b, a = signal.butter(self.order, normal_cutoff, btype='low')
        self.b = b
        self.a = a

    # Apply the low-pass filter
    def __call__(self, data: Any) -> Any:
        return torch.Tensor(signal.filtfilt(self.b, self.a, data).copy())
